...
import numpy as np
import pandas as pd
# put this right after the torch/pt_tabular imports succeed
import torch
from omegaconf import DictConfig, ListConfig

# PyTorch 2.6+: allow-list OmegaConf containers for safe torch.load
try:
    torch.serialization.add_safe_globals([DictConfig, ListConfig])
except Exception as _e:
    print(f"[WARN] add_safe_globals failed (ok to continue if not needed): {_e}")
...
# optional libs
# -------- Optional libs (robust, left-aligned) --------
# ---- Optional libs -----------------------------------------------------------
# Gradient-boosters
...
# ---- TabTransformer (pytorch_tabular) optional import + compatibility shims ---
TABTRANSFORMER_AVAILABLE = False
TABTRANSFORMER_IMPORT_ERROR = None
...
try:
    ...
    def _safe_TabTransformerConfig(**kw):
        """
        Build a TabTransformerConfig robust to pytorch_tabular version changes.
        - Renames common synonyms -> canonical arg names
        - Drops unsupported kwargs for the installed version
        """
        renames = {
            ...
            "shared_embed": "share_embedding",
        }
        for old, new in renames.items():
            if old in kw and new not in kw:
                kw[new] = kw.pop(old)

        allowed = set(signature(TabTransformerConfig.__init__).parameters.keys())
        clean = {k: v for k, v in kw.items() if k in allowed}
        return TabTransformerConfig(**clean)

    def _safe_TrainerConfig(**kw):
        """
        Build a TrainerConfig compatible with the installed pytorch_tabular version.
        - Maps 'accelerator' -> 'gpus' if 'accelerator' is not supported
        - Drops unknown kwargs (e.g., enable_checkpointing, deterministic) on older versions
        """
        allowed = set(signature(TrainerConfig.__init__).parameters.keys())

        # Map accelerator -> gpus if needed (older Lightning stacks)
        if "accelerator" in kw and "accelerator" not in allowed:
            accel = kw.pop("accelerator")
            use_gpu = False
            try:
                use_gpu = (accel in ("cuda", "gpu")) and torch.cuda.is_available()
            except Exception:
                use_gpu = False
            if "gpus" in allowed:
                kw["gpus"] = 1 if use_gpu else 0

        # Drop args unknown to this version
        for maybe in ("enable_checkpointing", "deterministic"):
            if maybe in kw and maybe not in allowed:
                kw.pop(maybe, None)

        if "load_best" in allowed and "load_best" not in kw:
          kw["load_best"] = False

        clean = {k: v for k, v in kw.items() if k in allowed}
        return TrainerConfig(**clean)

    def _safe_DataConfig(target, continuous_cols, categorical_cols):
      """
      Force target to be List[str] for all pytorch_tabular versions.
      Newer OmegaConf schemas *require* a list; older versions accept it too.
      """
      tgt_list = target if isinstance(target, (list, tuple)) else [str(target)]
      return DataConfig(
          target=list(tgt_list),
          continuous_cols=list(continuous_cols) if continuous_cols is not None else [],
          categorical_cols=list(categorical_cols) if categorical_cols is not None else [],
      )
    def _safe_OptimizerConfig(**kw):
      """
      Make an OptimizerConfig that works across pytorch_tabular versions.
      - If 'lr' not supported, try 'learning_rate'
      - Else, pack (lr, weight_decay, etc.) into optimizer_params
      - Drop any unknown kwargs silently
      """
      allowed = set(signature(OptimizerConfig.__init__).parameters.keys())
      clean = {}

      # Extract common optimizer hyperparams the caller might pass
      lr = kw.pop("lr", None)
      weight_decay = kw.pop("weight_decay", None)

      # Pass through any other provided kwargs if supported (e.g., optimizer, lr_scheduler)
      for k, v in list(kw.items()):
          if k in allowed:
              clean[k] = v

      # Case A: this version accepts 'lr'
      if "lr" in allowed and lr is not None:
          clean["lr"] = lr
          if weight_decay is not None and "weight_decay" in allowed:
              clean["weight_decay"] = weight_decay

      # Case B: this version uses 'learning_rate'
      elif "learning_rate" in allowed and lr is not None:
          clean["learning_rate"] = lr
          if weight_decay is not None and "weight_decay" in allowed:
              clean["weight_decay"] = weight_decay

    # Case C: pack into optimizer_params (older API)
      elif "optimizer_params" in allowed:
          opt_params = {}
          if lr is not None:
              opt_params["lr"] = lr
          if weight_decay is not None:
              opt_params["weight_decay"] = weight_decay
          if "optimizer_params" in kw and isinstance(kw["optimizer_params"], dict):
              opt_params = {**kw["optimizer_params"], **opt_params}
          clean["optimizer_params"] = opt_params

      oc = OptimizerConfig(**clean)
      print(f"[DIAG] OptimizerConfig resolved -> { {k: getattr(oc, k, None) for k in ['optimizer','lr','learning_rate','optimizer_params','weight_decay']} }")
      return oc
    ...
    # Fall back cleanly when pytorch_tabular isn't available
    TabularModel = None
    TabTransformerConfig = None
    DataConfig = TrainerConfig = OptimizerConfig = ExperimentConfig = ModelConfig = None

    def _safe_TabTransformerConfig(**kw):
        raise RuntimeError("pytorch_tabular not available; cannot build TabTransformerConfig")

    def _safe_TrainerConfig(**kw):
        raise RuntimeError("pytorch_tabular not available; cannot build TrainerConfig")

    def _safe_OptimizerConfig(**kw):
        raise RuntimeError("pytorch_tabular not available; cannot build OptimizerConfig")

    TABTRANSFORMER_AVAILABLE = False
    TABTRANSFORMER_IMPORT_ERROR = repr(_e)
...
print(f"[DIAG] TABTRANSFORMER_AVAILABLE={TABTRANSFORMER_AVAILABLE}")
if not TABTRANSFORMER_AVAILABLE and TABTRANSFORMER_IMPORT_ERROR:
    print(f"[DIAG] TabTransformer import error: {TABTRANSFORMER_IMPORT_ERROR}")
print(f"[DIAG] SHAP_AVAILABLE={SHAP_AVAILABLE}")
# ============================================================
# Config
# ============================================================
...
def _auc_proba_scorer(est, X, y):
    # works for our PTTabTransformer and for classical models
    if hasattr(est, "predict_proba"):
        ...
        # last resort: treat predict as probability
...
class PTTabTransformer(BaseEstimator, ClassifierMixin):
    # Make sklearn treat this as a classifier (fixes CalibratedClassifierCV issue)
    ...
    def __init__(
        ...
        # ⚠️ store EXACT objects you receive (no list(...), no copies)
        self.cat_cols = cat_cols
        self.num_cols = num_cols
        ...
        # internal, derived copies (set in fit)
        self._cat_cols_list = None
        self._num_cols_list = None
    ...
    def fit(self, X: pd.DataFrame, y: np.ndarray):
        # Basic contract: expect a DataFrame so pytorch_tabular sees named cols
        assert isinstance(X, pd.DataFrame), "PTTabTransformer expects a pandas DataFrame"

        # Stash target column name for predict-time stubs
        self._target_name = "__target__"

        # Build training frame
        df = X.copy()
        df[self._target_name] = np.asarray(y, dtype=int)

        # Derive concrete lists (safe for sklearn clone rules)
        self._cat_cols_list = list(self.cat_cols) if self.cat_cols is not None else []
        self._num_cols_list = list(self.num_cols) if self.num_cols is not None else []

        # Data config
        data_config = _safe_DataConfig(
            target=self._target_name,
            continuous_cols=self._num_cols_list,
            categorical_cols=self._cat_cols_list,
        )

        # ---- Loss / output alignment ----
        # Use 2-logit output w/ CrossEntropyLoss. Add optional class-weights.
        loss_kwargs = None
        loss_name = "CrossEntropyLoss"
        if self.imbalance_mode == "cost_sensitive":
            pos = int((df[self._target_name] == 1).sum())
            neg = int((df[self._target_name] == 0).sum())
            if pos > 0 and neg > 0:
                w0 = 1.0
                w1 = float(neg / max(1, pos))
                loss_kwargs = {"weight": torch.tensor([w0, w1], dtype=torch.float32)}

        model_config = _safe_TabTransformerConfig(
            task="classification",
            loss=loss_name,
            ...
            # learning rate belongs here for pytorch_tabular
            learning_rate=self.lr,
        )

        trainer_config = _safe_TrainerConfig(
            batch_size=self.batch_size,
            max_epochs=self.max_epochs,
            early_stopping="valid_loss",
            early_stopping_patience=self.patience,
            auto_lr_find=False,
            accelerator=self._infer_device(),
        ...
        optimizer_config = _safe_OptimizerConfig(
            optimizer="AdamW",
            # LR is already in model_config; pass only weight_decay here
            weight_decay=self.weight_decay,
        )

        # Build and train the TabularModel
        ...
        # Optional: tiny diag print (safe across versions)
        try:
            dev = trainer_config.accelerator if hasattr(trainer_config, "accelerator") else "cpu/gpu-compat"
            print(f"[DIAG] PTTabTransformer device={dev}; epochs={self.max_epochs}; bs={self.batch_size}")
        except Exception:
            pass

        self._model.fit(train=df)

        # ---- Mark as a scikit-learn classifier (metadata) ----
    ...
    def predict_proba(self, X: pd.DataFrame):
        assert getattr(self, "is_fitted_", False), "Call fit first."
        df = X.copy()
        ...
        # ---- Version-robust positive-class prob column detection ----
        cols = list(preds.columns)
        found_col = None

        # 1) Prefer explicit positive-class probability (common modern schema)
        for c in cols:
            if c.endswith("1_probability") or c.endswith("_1_probability"):
                found_col = c
                break

        # 2) Common single-probability names (older versions)
        if found_col is None:
            for c in ("prediction_probability", "prediction_proba", "prediction_1"):
                if c in preds.columns:
                    found_col = c
                    break

        # 3) Fallback: if there is exactly one "*probability" column, use it
        if found_col is None:
            prob_like = [c for c in cols if c.endswith("probability")]
            if len(prob_like) == 1:
                found_col = prob_like[0]

        # 4) Last resort: sometimes "prediction" is already a probability
        if found_col is None and "prediction" in preds.columns:
            found_col = "prediction"

        if found_col is None:
            raise KeyError(f"No probability-like column found in predictions: {cols}")

        p = preds[found_col].values.astype(float)
        p = np.clip(p, 1e-7, 1 - 1e-7)
        return np.c_[1.0 - p, p]
    
    def decision_function(self, X: pd.DataFrame):
        # Some sklearn scorers prefer this if available
        proba = self.predict_proba(X)[:, 1]
        # map (0,1) -> (-inf, +inf) monotonically; logistic inverse is fine
        eps = 1e-7
...
def build_pipeline(imbalance_mode: str, estimator, cfg: Config) -> ImbPipeline:
    ...
    if isinstance(estimator, PTTabTransformer):
        ...
        pipe = ImbPipeline([("clf", est)])   # <-- use a literal list here
        # - setattr(pipe, "_estimator_type", "classifier")
        return pipe
    ...
    # --- ensure sklearn treats this as a classifier ---
...
def inner_cv(
    ...
    search = RandomizedSearchCV(
      ...
      param_distributions=(grid or {}),
      n_iter=(CFG.N_TRIALS if grid else 1),
      scoring=_auc_proba_scorer,
      cv=inner,
      n_jobs=-1,
      random_state=random_state,
      refit=True,
      verbose=0,
      error_score="raise"  # <— add this temporarily
)
...
def nuke_tabtx_artifacts():
    base = runs_dir()
    if not os.path.isdir(base):
        return
    for fn in os.listdir(base):
        if re.match(r"TabTransformer__.*__fold\d+\.json$", fn):  # per-fold checkpoints
            os.remove(os.path.join(base, fn))
        if re.match(r"TabTransformer__.*__fold_metrics\.csv$", fn):  # per-model CSVs
            os.remove(os.path.join(base, fn))
    # per-(model,imbalance)/fold sidecar dirs
    for dn in os.listdir(base):
        if dn.startswith("TabTransformer__"):
            shutil.rmtree(os.path.join(base, dn), ignore_errors=True)
    print("[CLEAN] Removed old TabTransformer checkpoints & artifacts.")

nuke_tabtx_artifacts()
# ============================================================
...
def outer_cv(X: pd.DataFrame, y: np.ndarray):
    ...
    # Run only TabTransformer models
    specs = [
        s for s in make_model_specs(CFG)
        if s[0].startswith("TabTransformer__")
    ]
...
if __name__ == "__main__":
    ...
    # ------------------ Smoke test (acceptance pre-check) ------------------
    try:
        if TABTRANSFORMER_AVAILABLE:
            n_smoke = min(20, len(y_full))
            X_sm = X_full.iloc[:n_smoke].copy()
            y_sm = y_full[:n_smoke].copy()

            est_smoke = PTTabTransformer(
                cat_cols=cat_cols,
                num_cols=num_cols,
                random_state=CFG.RANDOM_STATE,
                max_epochs=5,
            ...
            est_smoke.fit(X_sm, y_sm)

            pp = est_smoke.predict_proba(X_sm[:5])
            assert pp.shape == (min(5, n_smoke), 2), f"predict_proba shape bad: {pp.shape}"

            # Platt if enough pos/neg, else identity – both must return proba
            cal = build_safe_calibrator(est_smoke, X_sm, y_sm)
            cal_pp = cal.predict_proba(X_sm[:5])
            assert cal_pp.shape == (min(5, n_smoke), 2), "calibrated predict_proba shape bad"

            print("[SANITY] PTTabTransformer smoke test OK: predict_proba & calibrator ✅")
        else:
            print("[SANITY] Skipped: TabTransformer not available.")
    except Exception as _e:
        print(f"[SANITY] PTTabTransformer smoke test FAILED: {_e}")
